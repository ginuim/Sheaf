...
import { AiEditHistorySection } from "./AiEditHistorySection";
import { AiEditPreviewCards } from "./AiEditPreviewCards";
import { collectMessageEditEntries } from "../lib/ai-edit-session";
...
type AiAgentPanelProps = {
  ...
  onFocusEditPreview?: (preview: AiAgentSessionPreview) => unknown;
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {messages.length === 0 ? (
            ...
            <ol className="m-0 grid list-none gap-3 p-0">
              {messages.map((message, messageIndex) => {
                ...
                let userPromptForAssistant: string | null = null;
                if (message.role === "assistant") {
                  for (let index = messageIndex - 1; index >= 0; index -= 1) {
                    ...
                    if (!text) continue;
                    userPromptForAssistant = text;
                    break;
                  }
                }
                const messageEditEntries =
                  message.role === "assistant" ? collectMessageEditEntries(message, userPromptForAssistant) : [];
                ...
                return (
                  <li className="mr-auto min-w-0 max-w-[86%]" key={message.id}>
                    <div className="grid gap-2">
                      ...
                      {messageEditEntries.length > 0 ? (
                        <AiEditPreviewCards
                          entries={messageEditEntries}
                          expandedEntryId={expandedEditEntryId}
                          translate={label}
                          onExpandedEntryIdChange={setExpandedEditEntryId}
                          onFocusPreview={onFocusEditPreview}
                        />
                      ) : null}
                    </div>
                  </li>
        ...
        <AiEditHistorySection messages={messages} translate={label} onFocusPreview={onFocusEditPreview} />
